...
export const useKeyword = () => {
  ...
  const encodeOption = computed(() => {
    ...
    if (keyOption.keywordType === 'or') {
      key = key ? splitBlank(key).map(text => `"${text}"`).join(" OR ") : key;
    } else {
      key = key ? `"${key}"` : key;
    }
    ...
    key = keyOption.minFavorite ? `${key} min_faves:${keyOption.minFavorite}` : key;
    key = keyOption.minRetweet ? `${key} min_retweets:${keyOption.minRetweet}` : key;
    key = keyOption.accountFilter ? `${key} filter:follows` : key;

    let exclude = '';
    if (keyOption.excludeKey) {
      exclude = splitBlank(keyOption.excludeKey).map(text => `-"${text}"`).join(" ");
      key = `${key} ${exclude}`;
    }

    if (keyOption.mentionTo) {
      key = keyOption.mentionTo.startsWith('@') ? `${key} ${keyOption.mentionTo}` : `${key} @${keyOption.mentionTo}`;
    }

    if (keyOption.filterSelected.length) {
      const filters = keyOption.filterSelected.map(text => `filter:${text}`).join(' ')
      key = `${key} ${filters}`;
    }

    if (keyOption.containHashTag) {
      const hash = splitBlank(keyOption.containHashTag).map(text => text.startsWith('#') ? text : `#${text}`).join();
      key = `${key} ${hash}`;
    }
